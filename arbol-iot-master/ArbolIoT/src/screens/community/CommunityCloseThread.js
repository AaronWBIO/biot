...
class CommunityCloseThread extends Component {
    ...
    _saveEntity(){
        var communityEntity = {
            bucketName: "community-publication",
            objectName: "close-reason",
        ...
        .then((res)=> {
            if(res.status == 200 || res.status == 201){
                this.updateThread(this.state.id);
            } else {
                this.setState({
                    showAlertRetry:true
                });
    ...
    updateThread(threadId){
        CommunityService.toggleStatus(
            threadId
        )
        .then(res => {
            if(res.status == 200 || res.status == 201){
                this.setState({showSuccessAlert:true, alertSuccessText:strings.communityCloseThread.closed});
            }
        })
        .catch(e => {});  
    }
        ...
        return (            
                <View key={item.id} >      
                    <ListItem style={{borderBottomWidth:0, marginLeft: 0,}}>
                        ...
                        <Body>
                            <Text style={{marginLeft:5, color:'#737373', fontSize:14}}>{strings.closeReasons[item.name]}</Text>
                        </Body>
                    </ListItem>
                </View>
        );
                ...
                <ScrollView>                    
                    <View style={{  }}>
                        <View style={{marginTop:40, alignItems: 'center', marginHorizontal: '15%', marginBottom: 20, }}>
                            ...
                            <Text style={ globalStyles.alertTitleMain}>{strings.communityCloseThread.title}</Text>
                            <Text style={[ globalStyles.alertText, {marginTop: 10}] }>{strings.communityCloseThread.message}</Text>
                        </View>
                        <Form style={{marginHorizontal:'15%', marginBottom: 20,}}>
                            ...
                            />
                            { this.state.last ? 
                                <ListItem style={{borderBottomWidth:0, marginLeft: 0, paddingBottom: 0, paddingTop:20, paddingRight:0,}}>
                                    <Textarea style={{borderBottomWidth: 1, borderBottomColor: '#CFCECF', marginTop: 0, width: '100%', color: '#737373', fontSize: 12}} 
                                        rowSpan={2} 
                                        placeholder={ strings.communityCloseThread.other } 
                                        ref="reason"
                        ...
                        <View style={styles.buttonContainer}>
                            <Button block success style={ styles.closeButton } onPress={ () => this.handleCloseThread() } >
                                <Text style= { styles.close }>{ strings.communityCloseThread.close }</Text>
                            </Button>
                        </View>
...
const mapStateToProps = (state) => {
    return {
        ...
        user: state.user,
        config: state.config.application.dynamic.communityModule.closeReasons,
...
export default connect(mapStateToProps, mapDispatchToProps)(CommunityCloseThread);